...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  const artifactPath = getArtifactPath(jobId, "html");
  if (!artifactPath) {
    return NextResponse.json({ error: "HTML artifact not found." }, { status: 404 });
  }
  ...
  return new NextResponse(content, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${job.sourceFilename.replace(/\.docx$/i, "")}.html"`
    }
  });
}
